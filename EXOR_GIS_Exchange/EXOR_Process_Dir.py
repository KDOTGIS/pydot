...
This geodatabase contains CANSYS items for roadway attributes that apply to a specific direction of travel especially on divided roadways
...
def DissolveDirectionalItems(OpEnvironmentMode):
    
    OpRunIn= OpEnvironment.OpRunInDSum  # @UndefinedVariable
    ...
    FCFieldsIgnore = ["OBJECTID", "CRND_RTE", "XSP", "SHAPE", "SHAPE.STLength()",
    ...
    for Item in FCList:
        ...
        if Exists(OpRunFullOut+ItemOut):  
            ...
            DisconnectUser(adm, "readonly")
            print "disconnected Readonly user and overwriting the data for "+ ItemOut
            AcceptConnections(adm, True)
            Dissolve_management(Item, OpRunFullOut+ItemOut, DissolveFields, FCGlobalFieldsSummarize, "MULTI_PART", "DISSOLVE_LINES")
            ChangePrivileges_management(OpRunFullOut+ItemOut, "readonly", "GRANT", "AS_IS")
        try:
...
if __name__== "__main__":
    ...
    DissolveDirectionalItems(OpEnvironment)
